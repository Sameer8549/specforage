...
@pytest.mark.asyncio
async def test_low_score_top_candidate_is_routed_to_review_without_llm() -> None:
    ...
    assert result.classify.unspsc_code is None
    assert {flag.code for flag in result.classify.flags} == {
        "classification_low_score",
        "classification_unresolved",
    }


@pytest.mark.asyncio
async def test_close_tie_still_reports_all_ten_deterministic_candidates() -> None:
    records = [
        UNSPSCRecord(
            "52000000",
            "Consumer Products",
